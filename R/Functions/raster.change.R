...
#' @param stat           Statistic to use in comparison ("kappa", "t.test", "cor", "entropy")
...
#'
#' @return A raster layer or stack object with the following layers:
#' \itemize{ 
...
#'  } 
#'
#' @note This function provides a Cohen's Kappa Weighted Kappa, paired t-test, delta entropy or
#'       rank correlation to compare two classified maps. Kappa and ttest values < 0 are reported as 0.      
#' @note For a weighted kappa, a matrix must be provided that correspond to the pairwise weights for 
#'         all values in both rasters.
#' @note Delta entropy is derived by calculating Shannon's on each focal window then subtracting them (e(x) - e(y))
#' 
...
#'	  
#'	s = 5    
#'  ( r.kappa <- raster.change(r1, r2, d = s) )   
#'  ( r.ttest <- raster.change(r1, r2, d = s, stat="t.test") )
#'  ( r.ent <- raster.change(r1, r2, d = s, stat="entropy") )   
#'  ( r.cor <- raster.change(r1, r2, d = s, stat="cor") )
#'  ( r.ce <- raster.change(r1, r2, d = s, stat = "cross-entropy") )	  
#'  ( r.kl <- raster.change(r1, r2, d = s, stat = "divergence") )	
#'      
#'  par(mfrow=c(3,2))
...
raster.change <- function(x, y, d = c(3,3), stat = c("kappa", "wkappa", "t.test",  
                          ...
                          out.raster = NULL, force.memory = FALSE) {
	stat = stat[1]
      options(warn=-1)	
  ...
  r <- stack(x,y)
  if (!canProcessInMemory(x) | !is.null(out.raster)) {
    if(force.memory) { pm = TRUE } else { pm = FALSE }
	if(is.null(out.raster)) out.raster = "xxxx01.tif"
	  if(stat == "t.test") {
        out <- writeStart(r, out.raster, overwrite=TRUE)
	  } else {
        out <- writeStart(r[[1]], out.raster, overwrite=TRUE)
      }	  
  } else {
    pm = TRUE
	  if(stat == "t.test") {
  ...
  # process in memory	
  if(pm == TRUE) {
    v <- getValuesFocal(r[[1:2]], 1, nrow(r[[1]]), ngb = d, 
	                    array = FALSE)
    mi <- rep(NA,nrow(v[[1]]))
